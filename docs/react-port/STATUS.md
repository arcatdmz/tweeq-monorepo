...
- **Docs content ported** to the React demo behind a hash router: Home is the default, the React-first setup includes an optional Vue reference, and the component, example, colors, features, UIST 2025, user-study, user-study-components, and presentation pages are available as React routes. `mdi-*` icon names normalized to `mdi:*`.
...
- **Docs site reproduced** in the demo: Home/Features/Components/Colors/Example tabs port the VuePress pages' text and demos; the auto-discovered gallery lives under "All Components", while `/` now opens Home. VuePress-style chrome: logo navbar, GitHub link, working light/dark toggle (theme store), per-page sidebar of h2 anchors with scroll tracking, pointer cursors on all clickables (Viewport reset sets cursor:inherit — demo.css overrides).
