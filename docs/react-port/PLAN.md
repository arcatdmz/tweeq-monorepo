...
- [x] Batch 8: panes/layout/complex — Tabs, ParameterGrid, InputComplex, PaneModal, PaneModalComplex, PaneModalTabs, PaneSplit, PaneExpandable, PaneFloating, PaneZUI, TitleBar, App, final TweeqProvider wiring
- [x] Phase 4: build config (lib entry `src/react/index.ts`), demo covers all components, dependency cleanup (drop vue deps + unused: @vueuse/gesture, monaco-themes, fp-ts, @material/material-color-utilities), final lint/typecheck/e2e
