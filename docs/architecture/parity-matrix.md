...
| Styling | scoped Stylus per SFC + shared stable parts | CSS Modules per component + shared stable parts | **in progress**: `@tweeq/styles/shared.styl` owns InputButton, InputButtonToggle, and InputCheckbox through stable component/part/state attributes; remaining renderer-local rules are tracked by MF-044 |
