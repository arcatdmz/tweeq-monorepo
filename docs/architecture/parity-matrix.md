...
| Pinia stores in `packages/vue/src/stores/` duplicating `@tweeq/dom` store factories | upstream legacy, kept in Stage V1 | replaced one store at a time by injected shared instances behind a `useTweeq()` facade (Stage V3); no `from 'pinia'` in the five app-level store adapters | **done 2026-07-13** for actions, appConfig, modal, multiSelect, and theme; component-local Pinia contexts remain separate V4 items |
