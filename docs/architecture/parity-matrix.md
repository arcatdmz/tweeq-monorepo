...
| InputNumber | SFC | ✅ | `inputNumber` | shared contract + docs-pages, number-vectors | controlled edits, expressions, configured steps, disabled state, zero-safe shared scrub/precision transition, stable range/grip/scale parts, and canonical styling aligned |
...
| InputRotary | SFC | ✅ | `inputRotary` | shared contract + core controller + rotary-snap + temporal | **done 2026-07-13**: continuous snap transition, shortest-angle and overlay geometry, native disabled/invalid state, controlled indicator, grouped positions, matching pointer hit areas, stable parts, and canonical CSS are aligned |
...
| Menu | SFC | ✅ | `menu` | shared contract + core safe-corridor/focus geometry + overlay | command/short-label/separator rendering, disabled semantics, menu roles, roving Arrow/Home/End focus, Enter/Space activation, submenu enter/return, Escape/close lifecycle, pointer corridor, shared public item types, and stable parts aligned in Phase 4 |
| MonacoEditor | SFC | ✅ | — | dropdown-code | `monaco-editor-vue3` vs `@monaco-editor/react` |
| MultiSelectPopup | SFC | ✅ | DOM multi-select controller | DOM controller fixtures + multi-select, overlay | one shared selection/action controller; capture/update/confirm/dispose, compatible actions, bounded relative Arrow/Home/End/Page movement, Shift/Alt steps, scalar/two-axis semantics, Escape rollback, keyboard-visible expansion, feature-safe popover fallback, stable action states, and canonical styling aligned |
...
| PaneFloating | SFC | ✅ | `panes` | shared contract + core anchor controller + layout-modals | position persistence, edge/minimize transitions, titlebar-aware viewport clamp, composed dimension correction, shared pointer lifecycle, stable parts, and canonical anchor/resize/minimized styling aligned |
...
| PaneSplit | SFC | ✅ | `panes` | shared contract + core resize controller + layout-modals | proportional/fixed first/fixed second movement, public minimum clamp, persistence, shared pointer lifecycle, focus-visible separator semantics, orientation-aware Arrow/Home/End resizing, ARIA values, and stable parts aligned in Phase 4 |
...
| SvgIcon | SFC | ✅ | `svgPath` | — | |
| Tabs | SFC | ✅ | `tabs` | shared contract + core selection/focus + layout-modals | enabled persisted/default/fallback selection, custom storage keys, native disabled buttons, automatic orientation-aware Arrow/Home/End activation, roving focus, stable tab/panel labelling, active/click lifecycle, id re-registration, ARIA, and stable parts aligned in Phase 4 |
...
| Styling | stable renderer parts consuming canonical CSS | stable renderer parts consuming canonical CSS | **done 2026-07-13**: `@tweeq/styles/style.css` owns all reset/component rules and includes the common Monaco base before Tweeq overrides; both renderer export aliases are byte-identical copies protected by build, baseline, and packed-artifact gates |
...
| `@tweeq/styles/{common,shared}.styl` source exports and app-wide Vite mixin injection | Phase 2 relocation | renderer-local Stylus consumers are gone; only `@tweeq/styles` itself compiles the source graph | **done 2026-07-13**: unused source entries and injection helper removed; the packed package exposes only the compiled `style.css` artifact |
| Legacy `docs/.vuepress` authoring source and `tweeq` TypeScript alias | upstream Vue docs | current dual-renderer docs/research pages own every active route; retain only assets/styles still imported by `apps/docs` | **partially retained intentionally**: all dormant components now import `@tweeq/vue` and the unscoped alias is deleted; shared logo, research assets, and user SCSS remain while the current docs app references them |
...
| 5. popover, tooltip, menus, command palette, multi-select | **done 2026-07-14** | Menu and Popover shared contracts run against both renderers; Tooltip and MultiSelect state machines are DOM-owned with controller fixtures; MultiSelect's scalar/two-axis keyboard actions run against both real galleries; CommandPalette history/navigation is core-owned and actions preserve shared lifecycle hooks; overlay/dropdown/multi-select Playwright coverage passes |
