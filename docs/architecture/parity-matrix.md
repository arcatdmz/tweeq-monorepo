...
| Styling | scoped Stylus per SFC + shared stable parts | CSS Modules per component + shared stable parts | **in progress**: `@tweeq/styles/shared.styl` owns the first primitive/control families (buttons, switches, indicators, icons/canvas, angle/group/grid wrappers) through stable component/part/state attributes; remaining renderer-local rules are tracked by MF-044 |
